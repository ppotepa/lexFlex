...
#[test]
fn english_where_question_maps_to_location_relation() {
    let il = api().parse("Where is Paris?", "en").expect("question should parse");
    let Interlingua::Natural(utterance) = il else { panic!("expected natural interlingua") };
    let question = utterance.sentences[0].question.as_ref().expect("question semantics");
    assert_eq!(question.kind, QuestionKind::Where);
    assert_eq!(question.proposition.predicate.0, "LOCATED_IN");
    assert_eq!(question.projection, QueryProjection::Object);
}
