...
pub fn run(options: ChatOptions) -> Result<(), Box<dyn Error>> {
    ...
    session.push_system_message(format!(
        "lexFlex chat ready. data root: {}. source policy: {}. offline: {}. Activity has one fixed panel; Tab switches focus, Enter/+/- control the focused tree, Alt+M toggles application mouse capture, Ctrl+Shift+C copies the selected entry, and Ctrl+Shift+B copies the transcript.",
